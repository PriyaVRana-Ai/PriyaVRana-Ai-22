      ...
      <div className="mx-auto max-w-3xl space-y-5">
        {messages.map((message, index) => (
          <div key={index} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[85%] whitespace-pre-wrap rounded-2xl px-4 py-3 ${
              message.role === "user" ? "bg-blue-600 text-white" : "border border-blue-500/30 bg-[#0b234d] text-blue-50"
            }`}>
              {message.content}
